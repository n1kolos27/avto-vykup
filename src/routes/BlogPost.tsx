...
const BlogPost: React.FC = () => {
  ...
  const formatContent = (content: string) => {
    ...
    return lines.map((line, index) => {
      if (line.match(/^\d+\./)) {
        return (
          <p key={index} className="text-base text-neutral-700 leading-7 mb-2 pl-4">
            {line}
          </p>
        );
        ...
        return (
          <h3 key={index} className="text-2xl font-bold text-neutral-900 mt-6 mb-3">
            {line}
          </h3>
        );
      } else {
        return (
          <p key={index} className="text-base text-neutral-700 leading-7 mb-4">
            {line}
          </p>
        );
      }
    ...
    <div className="flex-1 bg-neutral-50">
      ...
      <div className="max-w-[900px] w-full mx-auto px-4">
        ...
        <button
          ...
          className="mb-6 py-2 text-base text-primary-600 font-medium hover:text-primary-700 transition-colors"
        >
          ← Вернуться к статьям
        </button>
