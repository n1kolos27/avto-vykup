        ...
        <div className="overflow-x-auto mb-8">
          <div className="flex flex-row gap-3 px-1">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                className={`px-5 py-3 rounded-lg border-2 min-w-[200px] min-h-[44px] relative transition-all ${
                  activeTab === tab.id
                ...
                } ${tab.isRecommended ? 'border-success-500 dark:border-success-400' : ''}`}
                onClick={() => setActiveTab(tab.id)}
                aria-label={tab.label}
                aria-pressed={activeTab === tab.id}
              >
                {tab.isRecommended && (
                  <span className="absolute -top-2 right-2 text-xs font-bold text-success-500 dark:text-success-400 bg-white dark:bg-neutral-800 px-2 py-0.5 rounded">
                    РЕКОМЕНДУЕМ
                  </span>
                )}
                <span
          ...
          <div className="flex flex-col gap-6">
            {currentData.pros.length > 0 && (
              ...
              {currentData.pros.map((pro, index) => (
                <div key={index} className="flex flex-row items-start gap-3 mb-3">
