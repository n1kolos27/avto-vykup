...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  ...
  isError?: boolean;
  magnetic?: boolean;
  loadingProgress?: number; // 0-100 for progress indication
  children?: ReactNode;
  title?: string;
  testID?: string;
}
  ...
  return (
    ...
    >
      {ripples.map((ripple) => (
        <span
          key={ripple.id}
          className="absolute rounded-full bg-white/50 dark:bg-white/30 ripple pointer-events-none"
          style={{
            left: `${ripple.x}px`,
            top: `${ripple.y}px`,
            width: '0',
        ...
        <>
          ...
          {loadingProgress !== undefined && (
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/20 rounded-b-lg overflow-hidden">
              <div
                className="h-full bg-white/60 progress-bar-animated"
