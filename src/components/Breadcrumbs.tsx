...
const Breadcrumbs: React.FC<BreadcrumbsProps> = ({ items, className = '' }) => {
  ...
  return (
    <nav className={`flex flex-row items-center flex-wrap py-3 px-4 gap-2 ${className}`} aria-label="Хлебные крошки">
      {breadcrumbItems.map((item, index) => (
        <React.Fragment key={item.href}>
          {index > 0 && <span className="text-sm text-neutral-400 mx-1">›</span>}
          {index === breadcrumbItems.length - 1 ? (
            <span className="text-sm text-neutral-600 font-medium" aria-current="page">{item.label}</span>
          ) : (
            <button
