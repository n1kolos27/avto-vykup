 ...
 */
export function getSchemaForRoute(pathname: string): Array<Record<string, unknown>> {
  ...
  switch (pathname) {
    case '/': {
      ...
      const serviceSchema = {
        ...
        description:
          'Профессиональный выкуп автомобилей в Москве и Московской области. Срочный выкуп за 2 часа, быстрая оценка за 5 минут, честная рыночная цена, моментальная оплата.',
        provider: {
          '@type': 'Organization',
          name: 'Выкуп авто | Московский Авто Альянс',
          url: baseUrl,
    ...
    case '/calculator': {
      const webApplicationSchema = {
        ...
        description:
          'Онлайн калькулятор для расчета предварительной стоимости автомобиля. Учитывает марку, модель, год выпуска, пробег и состояние.',
 ...
 */
export function generateSchemaHTML(schemas: Array<Record<string, unknown>>): string {
  return schemas
    .map((schema) => {
