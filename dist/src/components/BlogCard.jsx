...
const BlogCard = ({ post }) => {
    ...
    return (<Card className="p-0 overflow-hidden h-full">
      <button onClick={() => navigate(`/blog/${post.slug}`)} className="flex flex-col h-full w-full text-left hover:opacity-90 transition-opacity">
        <div className="h-48 bg-primary-600 overflow-hidden">
          {post.image ? (<OptimizedImage src={post.image} alt={`Изображение статьи: ${post.title}`} className="w-full h-full object-cover" loading="lazy"/>) : (<div className="w-full h-full bg-primary-600 dark:bg-primary-500 flex items-center justify-center">
