    ...
    return (<section className="bg-neutral-50 dark:bg-neutral-900 py-16 px-4 md:py-20 md:px-6 transition-colors relative">
      ...
      <div className="max-w-[1200px] w-full mx-auto">
        ...
        <div className="flex flex-col gap-4">
          {faqs.map((faq, index) => (<AnimatedCard key={index} delay={index * 50}>
              <Card hover className="overflow-hidden">
              <button className="flex flex-row justify-between items-center p-5 w-full text-left hover:bg-neutral-50 dark:hover:bg-neutral-700 transition-colors" onClick={() => toggleFAQ(index)} aria-expanded={openIndex === index} aria-label={`${faq.question}. Нажмите для ${openIndex === index ? 'закрытия' : 'открытия'} ответа`}>
                <span className="flex-1 text-lg font-semibold text-neutral-900 dark:text-neutral-100 mr-4">
                  {faq.question}
                </span>
                <span className={`text-base text-primary-600 dark:text-primary-400 transition-transform ${openIndex === index ? 'rotate-180' : ''}`}>
                  ▼
                </span>
              </button>
              {openIndex === index && (<div className="px-5 pb-5 border-t border-neutral-100 dark:border-neutral-700">
