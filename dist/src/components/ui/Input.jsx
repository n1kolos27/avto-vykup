...
const Input = forwardRef(({ label, error, success, helperText, leftIcon, rightIcon, className = '', required, multiline, rows = 4, floatingLabel = false, onChangeText, onChange, value, ...props }, ref) => {
    ...
    return (<div className="w-full mb-4">
        <div className="relative w-full">
          {floatingLabel && label && (<label className={`absolute left-4 transition-all duration-300 cubic-bezier(0.4, 0, 0.2, 1) pointer-events-none ${isFloatingActive
                ? 'top-2 text-xs text-primary-600 dark:text-primary-400 font-medium scale-100'
                : 'top-1/2 -translate-y-1/2 text-base text-neutral-500 dark:text-neutral-400 scale-100'} ${leftIcon ? 'left-10' : ''} ${error ? 'text-error-500 dark:text-error-400' : ''} ${success ? 'text-success-500 dark:text-success-400' : ''}`}>
              {label}
              {required && <span className="text-error-500 dark:text-error-400"> *</span>}
            </label>)}
          {!floatingLabel && label && (<label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
              {label}
              {required && <span className="text-error-500 dark:text-error-400"> *</span>}
            </label>)}
          {leftIcon && !multiline && (<div className="absolute left-3 top-1/2 -translate-y-1/2 z-10 pointer-events-none">
              {leftIcon}
            </div>)}
          {multiline ? (<textarea ref={ref} className={textareaClasses} aria-label={label} aria-invalid={error ? 'true' : 'false'} aria-describedby={error || helperText ? `${props.id || 'input'}-helper` : undefined} onChange={handleChange} onFocus={handleFocus} onBlur={handleBlur} rows={rows} value={value} {...props}/>) : (<>
              <input ref={ref} className={`${inputClasses} min-h-[48px]`} aria-label={label} aria-invalid={error ? 'true' : 'false'} aria-describedby={error || helperText ? `${props.id || 'input'}-helper` : undefined} onChange={handleChange} onFocus={handleFocus} onBlur={handleBlur} value={value} {...props}/>
              {rightIcon && !error && !success && (<div className="absolute right-3 top-1/2 -translate-y-1/2 z-10 pointer-events-none">
                  {rightIcon}
                </div>)}
            </>)}
          {error && !multiline && (<div className="absolute right-3 top-1/2 -translate-y-1/2 z-10 pointer-events-none">
              <span className="text-xl text-error-500" aria-label="Ошибка">⚠️</span>
            </div>)}
          {success && !error && !multiline && (<div className="absolute right-3 top-1/2 -translate-y-1/2 z-10 pointer-events-none">
              <span className="text-xl text-success-500 success-check" aria-label="Успешно">✓</span>
            </div>)}
        </div>
        {error && (<p id={`${props.id || 'input'}-helper`} className="text-xs text-error-500 dark:text-error-400 mt-1 animate-slide-up" role="alert">
            {error}
          </p>)}
        {helperText && !error && (<p id={`${props.id || 'input'}-helper`} className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
            {helperText}
          </p>)}
      </div>);
});
