...
const Calculator = () => {
    ...
    return (<Card className="p-6 m-4">
      ...
      <div className="flex flex-col">
        <div className="mb-6">
          <div className="mb-4">
            <Input label="Марка автомобиля *" value={formData.brand} onChangeText={(value) => updateField('brand', value)} placeholder="Например: Toyota" error={errors.brand}/>
          </div>

          <div className="mb-4">
            <Input label="Модель *" value={formData.model} onChangeText={(value) => updateField('model', value)} placeholder="Например: Camry" error={errors.model}/>
          </div>

          <div className="flex flex-row gap-4 mb-4">
            <div className="flex-1">
              <Input label="Год выпуска *" value={formData.year.toString()} onChangeText={(value) => updateField('year', parseInt(value) || 0)} type="number" placeholder="2020" error={errors.year}/>
            </div>

            <div className="flex-1">
              <Input label="Пробег (км) *" value={formData.mileage.toString()} onChangeText={(value) => updateField('mileage', parseInt(value) || 0)} type="number" placeholder="50000" error={errors.mileage}/>
            </div>
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-row flex-wrap gap-2">
              {conditions.map((cond) => (<button key={cond.value} className={`px-4 py-2.5 rounded-lg border min-h-[44px] flex items-center justify-center transition-colors ${formData.condition === cond.value
                ? 'bg-primary-600 border-primary-600 text-white font-semibold'
                : 'bg-white dark:bg-neutral-800 border-neutral-300 dark:border-neutral-700 text-neutral-700 dark:text-neutral-200'}`} onClick={() => updateField('condition', cond.value)}>
          ...
          <div className="flex flex-row gap-4 mb-4">
            <div className="flex-1">
              <Input label="Объем двигателя (л)" value={formData.engineVolume?.toString() || ''} onChangeText={(value) => {
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-row flex-wrap gap-2">
                {transmissions.map((trans) => (<button key={trans.value} className={`px-4 py-2.5 rounded-lg border min-h-[44px] flex items-center justify-center transition-colors ${formData.transmission === trans.value
                ? 'bg-primary-600 border-primary-600 text-white font-semibold'
                : 'bg-white dark:bg-neutral-800 border-neutral-300 dark:border-neutral-700 text-neutral-700 dark:text-neutral-200'}`} onClick={() => updateField('transmission', trans.value)}>
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-row flex-wrap gap-2">
              {drives.map((d) => (<button key={d.value} className={`px-4 py-2.5 rounded-lg border min-h-[44px] flex items-center justify-center transition-colors ${formData.drive === d.value
                ? 'bg-primary-600 border-primary-600 text-white font-semibold'
                : 'bg-white dark:bg-neutral-800 border-neutral-300 dark:border-neutral-700 text-neutral-700 dark:text-neutral-200'}`} onClick={() => updateField('drive', d.value)}>
        ...
        {result ? (<div className="bg-primary-50 p-6 rounded-xl border-2 border-primary-200 mt-6">
            ...
            <button onClick={() => setShowDetails(!showDetails)} className="p-3 mb-4 text-center">
              <span className="text-sm font-semibold text-primary-600">
                {showDetails ? 'Скрыть детали' : 'Показать детали расчета'}
              </span>
            </button>

            {showDetails && (<div className="bg-white dark:bg-neutral-800 p-4 rounded-lg mb-4">
            ...
            <div className="flex flex-col gap-3">
              <Button onClick={scrollToForm} className="mb-3">
          ...
          </div>) : (<div className="bg-neutral-50 dark:bg-neutral-800 p-8 rounded-xl border-2 border-dashed border-neutral-300 dark:border-neutral-700 mt-6 flex items-center justify-center">
