...
const CarEvaluationForm = () => {
    ...
    if (isSubmitted) {
        return (<Card className="p-8 flex flex-col items-center">
    ...
    return (<Card className="p-6 m-4 md:p-8 md:m-6">
      <div className="mb-6">
      ...
      <div className="flex flex-col gap-4">
        <div className="mb-4 relative">
          <Input label="Марка автомобиля" value={formData.brand} onChangeText={handleBrandChange} onBlur={() => {
            setTouched(prev => ({ ...prev, brand: true }));
            const error = validateField('brand', formData.brand);
            setErrors(prev => ({ ...prev, brand: error }));
        }} placeholder="Например: Toyota" error={errors.brand} success={touched.brand && !errors.brand && formData.brand.length > 0} floatingLabel required/>
          {showBrandSuggestions && brandSuggestions.length > 0 && (<div className="absolute top-full left-0 right-0 bg-white dark:bg-neutral-800 rounded-lg border border-neutral-200 dark:border-neutral-700 mt-1 z-10 shadow-md">
              {brandSuggestions.map((brand) => (<button key={brand} className="p-3 border-b border-neutral-100 dark:border-neutral-700 last:border-b-0 w-full text-left hover:bg-neutral-50 dark:hover:bg-neutral-700 transition-colors" onClick={() => {
        ...
        <div className="mb-4">
          <Input label="Модель" value={formData.model} onChangeText={(value) => {
            ...
            setErrors(prev => ({ ...prev, model: error }));
        }} placeholder="Например: Camry" error={errors.model} success={touched.model && !errors.model && formData.model.length > 0} floatingLabel required/>
        </div>

        <div className="flex flex-row gap-3 mb-4">
          <div className="flex-1">
            <Input label="Год выпуска" value={formData.year.toString()} onChangeText={(value) => setFormData(prev => ({ ...prev, year: parseInt(value) || currentYear }))} type="number" placeholder={currentYear.toString()} error={errors.year} floatingLabel required/>
          </div>
          <div className="flex-1">
            <Input label="Пробег (км)" value={formData.mileage.toString()} onChangeText={(value) => setFormData(prev => ({ ...prev, mileage: parseInt(value) || 0 }))} type="number" placeholder="0" error={errors.mileage} floatingLabel required/>
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-row flex-wrap gap-2">
            {conditions.map((condition) => (<button key={condition.value} className={`px-4 py-2.5 rounded-lg border min-h-[44px] flex items-center justify-center transition-colors ${formData.condition === condition.value
                ? 'bg-primary-600 border-primary-600 text-white font-semibold'
                : 'bg-white dark:bg-neutral-800 border-neutral-300 dark:border-neutral-700 text-neutral-700 dark:text-neutral-200'}`} onClick={() => setFormData(prev => ({ ...prev, condition: condition.value }))}>
        ...
        <div className="mb-4">
          <Input label="Ваше имя" value={formData.name || ''} onChangeText={(value) => setFormData(prev => ({ ...prev, name: value }))} placeholder="Иван" error={errors.name} floatingLabel/>
        </div>

        <div className="mb-4">
          <Input label="Телефон" value={formData.phone} onChangeText={(value) => {
            ...
            setErrors(prev => ({ ...prev, phone: error }));
        }} placeholder="+7 (999) 123-45-67" type="tel" error={errors.phone} success={touched.phone && !errors.phone && formData.phone.length >= 10} floatingLabel required/>
        </div>

        <Button onClick={onSubmit} isLoading={isLoading} disabled={isLoading || formProgress < 100} size="lg" className="mt-2">
          Получить оценку
        </Button>
      </div>
    </Card>);
