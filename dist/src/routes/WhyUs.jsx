    ...
    return (<div className="flex-1 bg-neutral-50">
      ...
      <div className="max-w-[1200px] w-full mx-auto px-4">
        ...
        <div className="flex flex-row flex-wrap gap-6 mb-8">
          {advantages.map((advantage, index) => (<Card key={index} className="flex-1 min-w-[300px] p-6">
              ...
              <div className="flex flex-col gap-2">
                {advantage.details.map((detail, idx) => (<div key={idx} className="flex flex-row items-start gap-2">
          ...
          <div className="flex flex-row gap-6">
            <div className="flex-1">
              ...
              <div className="flex flex-col gap-3">
                {['Сделка за 2 часа', 'Честная рыночная цена', 'Оплата сразу', 'Любое состояние', 'Оформляем документы'].map((item) => (<div key={item} className="flex flex-row items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-col gap-3">
                {['1-2 недели', 'Заниженная на 15-30%', 'Через несколько дней', 'Только в хорошем', 'Нужно делать самому'].map((item) => (<div key={item} className="flex flex-row items-start gap-3">
                    <span className="text-lg text-white font-bold mt-0.5">✗</span>
                    <span className="text-lg text-white flex-1 leading-7">{item}</span>
                  </div>))}
              </div>
