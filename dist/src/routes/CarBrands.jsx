      ...
      <div className="max-w-[1200px] w-full mx-auto px-4">
        ...
        <Card className="p-6 mb-8">
          <div className="flex flex-row gap-4 mb-4">
            <div className="flex-1 flex flex-row items-center border border-neutral-300 rounded-lg px-3">
              ...
              <Input value={searchQuery} onChangeText={setSearchQuery} placeholder="Поиск по марке..." className="border-0 p-0"/>
            </div>
          ...
          <div className="flex flex-row flex-wrap gap-2">
            {categories.map((category) => (<button key={category.value} className={`px-4 py-2 rounded-lg transition-colors ${selectedCategory === category.value
                ? 'bg-primary-600 text-white font-semibold'
                : 'bg-neutral-100 text-neutral-700 font-medium'}`} onClick={() => setSelectedCategory(category.value)}>
        ...
        <div className="flex flex-row flex-wrap gap-6 mb-8">
          {filteredBrands.length > 0 ? (filteredBrands.map((brand) => {
            ...
            return (<Card key={brand.name} className="flex-1 min-w-[300px] p-6">
                  ...
                  <div className="flex flex-row flex-wrap gap-2 mb-4">
                    {brand.examples.map((model, idx) => (<div key={idx} className="bg-neutral-100 px-2 py-1 rounded">
                        <span className="text-xs text-neutral-700">{model}</span>
                      </div>))}
                  </div>
                  <button onClick={() => navigate(`/services/buyback-cars?brand=${encodeURIComponent(brand.name)}`)} className="text-base font-semibold text-primary-600 hover:text-primary-700 transition-colors">
        ...
        <div className="bg-primary-600 rounded-xl p-8 mb-8 flex flex-col items-center">
          ...
          <div className="flex flex-row flex-wrap gap-4 justify-center">
            <Button onClick={() => navigate('/services/buyback-cars')} variant="secondary" className="min-w-[200px]">
              Получить оценку
            </Button>
            <Button onClick={() => {
            window.location.href = `tel:${APP_CONFIG.PHONE_1}`;
        }} variant="outline" className="min-w-[200px] bg-white/10 border-white text-white hover:bg-white/20">
        ...
        ].map((service) => (<button key={service.path} onClick={() => navigate(service.path)} className="flex-1 min-w-[200px] bg-neutral-50 rounded-lg p-4 flex flex-col items-center hover:bg-neutral-100 transition-colors">
