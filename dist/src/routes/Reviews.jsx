    ...
    return (<div className="flex-1 bg-neutral-50">
      <div className="max-w-[1200px] w-full mx-auto px-4">
        ...
        <div className="flex flex-col items-center mb-8 mt-4">
          ...
          <div className="flex flex-row items-center gap-2 mb-4">
            ...
            <div className="flex flex-row gap-1">
              {[...Array(5)].map((_, i) => (<span key={i} className={`text-2xl ${i < Math.round(averageRating) ? 'opacity-100' : 'opacity-30'}`}>
                  ⭐
                </span>))}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-4 mb-8">
          <div className="mb-4">
            ...
            <div className="flex flex-row flex-wrap gap-2">
              <button className={`px-4 py-2 rounded-lg min-h-[44px] flex items-center justify-center transition-colors ${filterRating === null
            ? 'bg-primary-600 text-white font-medium'
            : 'bg-neutral-200 text-neutral-700 font-medium'}`} onClick={() => setFilterRating(null)}>
                <span className="text-sm">Все</span>
              </button>
              {[5, 4, 3, 2, 1].map((rating) => (<button key={rating} className={`px-4 py-2 rounded-lg min-h-[44px] flex items-center justify-center transition-colors ${filterRating === rating
                ? 'bg-primary-600 text-white font-medium'
                : 'bg-neutral-200 text-neutral-700 font-medium'}`} onClick={() => setFilterRating(rating)}>
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-row flex-wrap gap-2">
              {categories.map((cat) => (<button key={cat.value || 'all'} className={`px-4 py-2 rounded-lg min-h-[44px] flex items-center justify-center transition-colors ${filterCategory === cat.value
                ? 'bg-primary-600 text-white font-medium'
                : 'bg-neutral-200 text-neutral-700 font-medium'}`} onClick={() => setFilterCategory(cat.value)}>
                  <span className="text-sm">{cat.label}</span>
                </button>))}
            </div>
          </div>

          <Button onClick={() => setIsFormOpen(!isFormOpen)} variant="primary" className="self-start">
            {isFormOpen ? 'Закрыть форму' : 'Оставить отзыв'}
          </Button>
        </div>

        {isFormOpen && (<Card className="p-6 mb-8">
            ...
            {isSubmitted ? (<div className="bg-success-100 border border-success-300 rounded-lg p-4">
                ...
                <div className="flex flex-row gap-4">
                  ...
                  <div className="flex-1">
                    <Input label="Модель автомобиля" value={formData.carModel || ''} onChangeText={(value) => setFormData((prev) => ({ ...prev, carModel: value }))} placeholder="Необязательно"/>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-row gap-2">
                    {[1, 2, 3, 4, 5].map((rating) => (<button key={rating} className={`w-11 h-11 rounded-full border-2 flex items-center justify-center transition-colors ${formData.rating === rating
                        ? 'border-warning-500 bg-warning-100'
                        : 'border-neutral-300 bg-white'}`} onClick={() => setFormData((prev) => ({ ...prev, rating }))}>
                        <span className="text-2xl">⭐</span>
                      </button>))}
                  </div>
                </div>

                <Input label="Ваш отзыв *" value={formData.text} onChangeText={(value) => setFormData((prev) => ({ ...prev, text: value }))} placeholder="Поделитесь своим опытом..." multiline rows={4} required/>

                {error && (<div className="bg-error-100 border border-error-300 rounded-lg p-4">
        ...
        <div className="flex flex-row flex-wrap gap-6 mb-12">
          {filteredReviews.map((review) => (<ReviewCard key={review.id} review={review} index={0}/>))}
        </div>

        {filteredReviews.length === 0 && (<EmptyState title="Отзывы не найдены" description="Нет отзывов с выбранным рейтингом или категорией. Попробуйте изменить фильтры." variant="info" actionLabel="Сбросить фильтры" onAction={() => {
