...
router.post('/evaluation', applyRateLimiting(RATE_LIMIT.EVALUATION), asyncHandler(async (req, res) => {
    ...
    if (!validation.isValid) {
        const error = createError(Object.values(validation.errors)[0] || 'Ошибка валидации данных', ERROR_CODES.VALIDATION_ERROR, HTTP_STATUS.BAD_REQUEST);
    ...
    const emailData = formatEvaluationEmail({
        ...
        condition: sanitized.condition || 'satisfactory',
    });
    // Отправляем email
    ...
    if (!emailResult.success) {
        ...
        if (ENV.NODE_ENV === 'production') {
            throw createError('Ошибка при отправке заявки. Попробуйте позже или свяжитесь с нами по телефону.', ERROR_CODES.INTERNAL_SERVER_ERROR, HTTP_STATUS.INTERNAL_SERVER_ERROR);
        }
...
// API: Контакты
router.post('/contact', applyRateLimiting(RATE_LIMIT.CONTACT), asyncHandler(async (req, res) => {
    const startTime = Date.now();
    const clientIP = getClientIP(req);
    const body = req.body;
    const sanitized = sanitizeInput(body);
    ...
    if (!sanitized.message || sanitized.message.length < 10) {
        throw createError('Сообщение слишком короткое (минимум 10 символов)', ERROR_CODES.VALIDATION_ERROR, HTTP_STATUS.BAD_REQUEST);
    }
    // Формируем email
    ...
    if (!emailResult.success) {
        ...
        // В production режиме возвращаем ошибку
        if (ENV.NODE_ENV === 'production') {
            throw createError('Ошибка при отправке сообщения. Попробуйте позже или свяжитесь с нами по телефону.', ERROR_CODES.INTERNAL_SERVER_ERROR, HTTP_STATUS.INTERNAL_SERVER_ERROR);
        }
        // В dev режиме логируем, но возвращаем success
        logger.warn('Email service not configured, but returning success in dev mode');
    }
...
// API: Отзыв
router.post('/review', applyRateLimiting(RATE_LIMIT.REVIEW), asyncHandler(async (req, res) => {
    const startTime = Date.now();
    const clientIP = getClientIP(req);
    const body = req.body;
    const sanitized = sanitizeInput(body);
    ...
    if (!sanitized.text || sanitized.text.length < 10) {
        throw createError('Отзыв слишком короткий (минимум 10 символов)', ERROR_CODES.VALIDATION_ERROR, HTTP_STATUS.BAD_REQUEST);
    }
    // Формируем email
    ...
    if (!emailResult.success) {
        ...
        // В production режиме возвращаем ошибку
        if (ENV.NODE_ENV === 'production') {
            throw createError('Ошибка при отправке отзыва. Попробуйте позже или свяжитесь с нами по телефону.', ERROR_CODES.INTERNAL_SERVER_ERROR, HTTP_STATUS.INTERNAL_SERVER_ERROR);
        }
        // В dev режиме логируем, но возвращаем success
        logger.warn('Email service not configured, but returning success in dev mode');
    }
