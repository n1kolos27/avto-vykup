...
import { generateMetadata, type Metadata } from '../src/lib/seo/metadata.js';
...
app.use((req: Request, res: Response, next: NextFunction) => {
  // В development режиме не используем nonce, чтобы разрешить unsafe-inline для Vite
  const nonce = isProduction ? ((req as { nonce?: string }).nonce || '') : '';
  helmet({
...
app.get('/api/health/ports', async (req: Request, res: Response) => {
  try {
...
app.get('/api/csrf-token', (req: Request, res: Response) => {
  const token = (res.locals as { csrfToken?: string }).csrfToken;
  if (token) {
...
let blogPostsCache: Record<string, { title: string; excerpt: string; category: string }> | null = null;

// Загрузка данных блог-постов при старте (асинхронно)
(async () => {
  try {
    const { blogPosts } = await import('../src/routes/BlogPost.js');
    blogPostsCache = Object.fromEntries(
 ...
 */
async function renderHTML(url: string, metadata?: Metadata): Promise<string> {
  ...
  try {
    ...
    const result = await renderApp({ url } as Request);
    ...
    if (validSchemas && validSchemas.length > 0) {
      const schemaHTML = generateSchemaHTML(validSchemas as Array<Record<string, unknown>>);
 ...
 */
function getFallbackHTML(metadata?: Metadata): string {
  return `<!DOCTYPE html>
<html lang="ru">
<head>
  <meta charset="UTF-8">
 ...
 */
function injectPreloadLinks(html: string): string {
  try {
 ...
 */
function injectPreconnectLinks(html: string): string {
 ...
 */
function injectDNSPrefetchLinks(html: string): string {
...
function getMetadataForRoute(pathname: string): Metadata {
